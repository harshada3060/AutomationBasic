...
public class ExtentDemo extends SeleniumUtility {
	...
	public static void starTest() {
		report = new ExtentReports(".\\ExetentReport\\ExtendReportResults.html");
		
	}

	@Test
	public void TestCase1() throws IOException {
		test = report.startTest("Google page validation-valid");
		...
		if (getCurrentTitleOfApplication().equals("Google")) {
			test.log(LogStatus.PASS, test.addScreenCapture(screenShot(driver)),
					"Navigated to the specified URL successfully and page tile is also validated");
			// System.out.println("Navigated to the specified URL successfully and page tile
			// is also validated");
		} else {
			test.log(LogStatus.FAIL, test.addScreencast(screenShot(driver)), "Google page validation got failed");
